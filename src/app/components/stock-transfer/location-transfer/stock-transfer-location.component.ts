    ...
    this._stockItemService.checkOutStockItem(this.itemCode.toUpperCase(), this.fromBin.toUpperCase()).subscribe(
      response => {
        if (response[2] >= this.quantity) {
          ...
          if (response[2] >= this.quantity) {
            //TODO: Ubicación fija aplica solo para IGB en ubicaciones de picking y MOTOZONE solo para bodega 32-SANBARTOLOME
            if (this.identity.selectedCompany === 'VARROC' && this.identity.warehouseCode === '13') {
              this.addItem();
            } else {
                  ...
                  if (response.code == 0) {
                    if (response.content[3] === 'PICKING') {
                      this._binLocationService.getLocationFixed(this.itemCode.toUpperCase()).subscribe(
                        response => {
                          if (response.content !== this.toBin.toUpperCase()) {
                            $('#modal_transfer_process').modal('hide');
